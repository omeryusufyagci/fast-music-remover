    ...
    def check_dependency(self, system, install):
        ...
        try:
            ...
            if install:
                ...
                print(f'{self.name} is not installed. Use --install to install dependencies automatically or install manually.')
                sys.exit(1)
        ...
        if 'all' in self.install_cmd:
            ...
            if system == 'Linux':
                ...
                if linux_distro in ['ubuntu', 'debian', 'kali']:
                    install_cmd = ['sudo apt-get update', f'sudo apt-get install -y {self.package_name.get(system, self.name)}']
...
dependencies = [
    ...
    Dependency('sndfile', {'Windows': 'mingw-w64-x86_64-libsndfile', 'Linux': 'libsndfile1-dev'}, {'all': 'pkg-config --exists sndfile'})
]
