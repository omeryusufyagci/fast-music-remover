...
FFmpegCommandBuilder& FFmpegCommandBuilder::addFilterComplex(
    const std::vector<fs::path> processedChunkCol) {
    addFlag("-filter_complex", buildFilterComplex(processedChunkCol));
...
std::string FFmpegCommandBuilder::buildFilterComplex(
    const std::vector<fs::path> processedChunkCol) const {
    // Build filter complex, i.e. a set of instructions for FFmpeg (called filter graph)
    std::string filterComplex = buildCrossfade(static_cast<int>(processedChunkCol.size()));
    return filterComplex;
}
